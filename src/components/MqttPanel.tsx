...
export const MqttPanel: React.FC<Props> = ({ options, data, fieldConfig, id, width, height }) => {
  ...
  useEffect(() => {
    switch (options.mode) {
      ...
      case 'Slider': {
        ...
        const def = min; // default to min when no explicit default property exists
        setStagedValue(def);
        break;
      }
      case 'Switch': {
        const def = false; // default to false when no explicit default property exists
        setStagedValue(def);
        break;
      }
      case 'Button':
  ...
  const publish = useCallback(() => {
    ...
    log('publish', { topic: publishTopic, payload });
    clientRef.current.publish(publishTopic!, payload, {
      qos: 0,
      retain: false,
    });
    setLastSent(payload);
  ...
  }, [options.mode, options.model, options.text, stagedValue, connected, publishTopic, publish]);
